...
export function Navbar({
  ...
  return (
    <nav className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-sm">
            ...
            {!isSignedIn ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="hidden sm:inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors shadow-sm"
                >
                  Sign Up
                </button>
              </div>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  {user.name ? user.name.slice(0, 1) : "Unknown"}
                </button>
      ...
      {/* Mobile Sign In/Sign Up (visible only on mobile) */}
      {!isSignedIn && (
        <div className="md:hidden border-t border-slate-200 px-4 py-3 bg-slate-50 flex gap-2">
          <button
            onClick={onSignInClick}
            className="flex-1 px-3 py-2 text-sm font-medium text-indigo-600 bg-white border border-indigo-200 rounded-lg hover:bg-indigo-50 transition-colors"
          >
            Sign In
          </button>
          <button
            onClick={onSignUpClick}
            className="flex-1 px-3 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"
          >
            Sign Up
          </button>
        </div>
